---
description: "Structured test case design session for HypermediaEngine. Takes an implementation (and acceptance criteria if available) and produces a complete test plan specifying what to test, how, and why — ready to hand off to write-tests."
agent: "agent"
argument-hint: "Path to the component to test, or paste acceptance criteria"
---

# Operating Methodology

You design test cases in three phases. Complete each phase fully before advancing. The output of this skill is a test plan — not test code. Code is written by the `write-tests` skill.
...
2. Read every public interface and method on the component under test — understand what each one promises.
3. If a requirements document or acceptance criteria were provided, read them fully. Map each AC to the method(s) that implement it.
4. If a surviving mutant report from `dotnet stryker` was provided, read it — each mutant is a gap in the existing test suite.
5. Locate the existing test projects. Read a sample of existing tests to understand naming conventions and fixture patterns.

---

## Phase 1 — Coverage Analysis (silent, then present)
...
Fill this table completely before moving on. A method with no row is untested by design.
...
Justify every integration test — default to unit tests.
...
- State the test case that would kill it
...
After presenting the plan, ask the user to confirm or request changes before handing off to `write-tests`.

---
description: "Create, modify, or delete GitHub Copilot prompt files and Claude command files across both platforms using a discovery-first, non-destructive workflow. Invoked by agent-manager for all command/prompt lifecycle operations."
agent: "agent"
argument-hint: "Operation: create <name> | modify <name> | delete <name> | sync <name> — platform: claude | copilot | both"
---

This skill creates and maintains Claude command files (`.claude/commands/*.md`) and GitHub Copilot prompt files (`.github/prompts/*.prompt.md`). It keeps both platforms in sync and defaults to non-destructive operations.

Every other agent must route command and prompt creation, modification, and deletion through the `agent-manager` agent rather than editing these files directly.

## Modes

### `create <name>`

Create a new command and prompt pair for a named skill:

1. Validate `<name>` is kebab-case; fail if both files already exist.
2. Write `.claude/commands/<name>.md` with proper frontmatter and Skill() usage block.
3. Write `.github/prompts/<name>.prompt.md` with proper frontmatter, modes, and workflow summary.
4. Report paths written and any warnings.

### `modify <name>`

Update an existing command/prompt pair:
...
Remove or archive a command/prompt pair:

1. Default to **soft delete**: rename to `<name>.md.deprecated` / `<name>.prompt.md.deprecated`.
2. Hard delete only with explicit flag and user confirmation.
3. Report what was archived or removed.

### `sync <name>`

Reconcile drift between the Claude command and Copilot prompt:

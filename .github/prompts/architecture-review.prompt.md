...
agent: "agent"
argument-hint: "Subsystem, layer, or PR/branch to review (or leave blank for full solution)"
---
...
2. Glob the solution structure: identify projects, layers, and their dependencies (`*.csproj`).
3. Read any `docs/architecture/` files and ADRs under `docs/architecture/decisions/`.
...
5. Determine the scope of the review: whole solution or a subsystem named in the argument.
...
- What is the primary concern driving this review (coupling, testability, performance boundaries, security surface)?
...
Perform the full architectural inspection against the following dimensions.

### 2.1 Layering and Dependency Direction

- Are dependencies flowing in the correct direction (outer layers depend on inner)?
...
- Are responsibilities leaking across boundaries (business logic in middleware, persistence logic in filters)?
...
- Are cross-cutting concerns (auth, logging, validation) handled consistently across the pipeline?
...
Produce an **Architecture Review Report** in planning mode:
...
List any unexpected or inverted dependencies found.

## Open Questions
Questions requiring business or team input before architectural decisions can be made.

## Handoff Notes for Architecture Design
If any finding warrants a new architectural design session, state it here.
```

After presenting the report, state in one sentence which Critical finding to address first.

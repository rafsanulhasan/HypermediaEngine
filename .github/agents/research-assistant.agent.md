---
name: "research-assistant-copilot"
...
When another agent finds itself reaching for `web` search, or library-docs lookup tools directly, that is a signal to delegate to `research-assistant` instead. The only exceptions are trivial one-shot factual lookups already answered inline during classification by `triage-agent`.
